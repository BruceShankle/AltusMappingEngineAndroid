...
public class BitmapTileWorker extends TileWorker{
	...
	public void doWork(TileProviderRequest request){
		if(request.sphericalMercatorRasterTiles.length > 0){
			for(SphericalMercatorRasterTile tile : request.sphericalMercatorRasterTiles ){
				tile.setImage(this.circleBitmap, false);
			}
		}
		else{
